...
export default function LanguagesPage() {
  ...
  const handleCreate = async (data: { name: string; code: string; flagUrl?: string }) => {
    try {
  ...
  const handleUpdate = async (data: { name: string; code: string; flagUrl?: string }) => {
    ...
    <div className="container mx-auto py-6 space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage available languages for the platform</p>
        </div>
        {isAdmin && (
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent>
              ...
              <LanguageForm onSubmit={handleCreate} onCancel={() => setIsCreateDialogOpen(false)} isLoading={isFormLoading} />
            </DialogContent>
          </Dialog>
        )}
      </div>
        ...
        <CardHeader>
          <div className="flex items-center gap-4">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input placeholder="Search languages..." value={searchInput} onChange={(e) => setSearchInput(e.target.value)} className="pl-10" />
            </div>
            <Badge variant="secondary">{pagination.total} total languages</Badge>
          </div>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <>
                ...
                <TableBody>
                  {languages.map((lang) => (
                    <TableRow key={lang.id}>
                      ...
                      <TableCell>{new Date(lang.createdAt).toLocaleDateString()}</TableCell>
                      <TableCell className="text-right">
                        <div className="flex justify-end gap-2">
                          {/* Edit */}
                          <Dialog open={editingLanguage?.id === lang.id} onOpenChange={(o) => !o && setEditingLanguage(null)}>
                            ...
                            <DialogContent>
                              ...
                              <LanguageForm language={lang} onSubmit={handleUpdate} onCancel={() => setEditingLanguage(null)} isLoading={isFormLoading} />
                            </DialogContent>
                          </Dialog>
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent>
                              ...
                              <AlertDialogFooter>
                                ...
                                <AlertDialogAction onClick={() => handleDelete(lang.id)} className="bg-red-600 hover:bg-red-700">
                                  Delete
                                </AlertDialogAction>
                              </AlertDialogFooter>
                            </AlertDialogContent>
              ...
              <div className="flex items-center justify-between mt-6">
                <p className="text-sm text-muted-foreground">
                  Showing {((page - 1) * 10) + 1} to {Math.min(page * 10, pagination.total)} of {pagination.total} languages
                </p>
                <div className="flex gap-2">
                  ...
                  <Button variant="outline" size="sm" onClick={() => setPage(page + 1)} disabled={page >= pagination.totalPages}>Next</Button>
                </div>
              </div>
            </>
          )}
