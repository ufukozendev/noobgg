...
export function useLanguages(options: UseLanguagesOptions = {}) {
  ...
  const fetchLanguages = async () => {
    try {
      ...
      const res = await fetch(`${base}/v1/languages?${params.toString()}`);
  ...
  const updateLanguage = async (
    ...
    const base = process.env.NEXT_PUBLIC_API_URL ?? "";
    const res = await fetch(`${base}/v1/languages/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
  ...
  const deleteLanguage = async (id: string) => {
    const base = process.env.NEXT_PUBLIC_API_URL ?? "";
    const res = await fetch(`${base}/v1/languages/${id}`, { method: "DELETE" });
    if (!res.ok) {
      const err = await res.json();
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        setLoading(true);
        const base = process.env.NEXT_PUBLIC_API_URL ?? "";
        const res = await fetch(`${base}/v1/languages/all`);
        if (!res.ok) throw new Error("Failed to fetch languages");
